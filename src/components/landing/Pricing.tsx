...
const Checkmark = () => (
  <svg className="w-[16px] h-[16px] flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
...
export default function Pricing() {
  ...
  const handleVerifyUtr = async (e?: React.FormEvent) => {
    ...
    if (!cleanInput || cleanInput.length < 4) {
      setUtrError("Please enter a valid UPI Reference / UTR Number (minimum 4 digits).");
      return;
    }
  ...
  const upiPayUri = upiModal
    ? `upi://pay?pa=${UPI_ID}&pn=Scribe.txt&am=${upiModal.amount}&cu=INR&tn=${encodeURIComponent(upiModal.plan.toUpperCase() + " Subscription")}`
    : "";
  ...
  return (
    <section className="section-pad relative" id="pricing">
      <div className="section-head text-center mb-10">
          ...
          <div className="bg-white border-2 border-ink rounded-full p-1 shadow-[2px_2px_0_#171717] inline-flex items-center gap-1 font-pt-narrow">
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-[15px] font-bold transition-all ${
                billingCycle === "monthly" ? "bg-indigo text-white shadow-[1px_1px_0_#171717]" : "text-text-gray hover:text-ink"
              }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-[15px] font-bold transition-all flex items-center gap-1.5 ${
                billingCycle === "yearly" ? "bg-indigo text-white shadow-[1px_1px_0_#171717]" : "text-text-gray hover:text-ink"
              }`}
            >
        ...
        {/* Logged-in user info */}
        {!authLoading && user && (
          <div className="mt-4 inline-flex items-center gap-2 bg-green-50 border border-green-300 rounded-full px-4 py-1.5 font-pt-narrow text-[13px] text-green-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-[24px] items-stretch max-w-[1040px] mx-auto">
        ...
        <div className="card-neo p-[32px] flex flex-col relative bg-white border-2 border-ink rounded-[20px] shadow-[4px_4px_0_#171717]">
          ...
          <ul className="flex flex-col gap-[12px] mb-[30px] flex-grow font-pt-narrow text-[17px] text-ink">
            <li className="flex items-center gap-[10px] font-bold text-indigo"><Checkmark /> 7 Free Transcripts Daily</li>
            <li className="flex items-center gap-[10px]"><Checkmark /> High-Accuracy Speech Recognition</li>
            <li className="flex items-center gap-[10px]"><Checkmark /> YouTube, Instagram & Files</li>
            <li className="flex items-center gap-[10px]"><Checkmark /> TXT, SRT & VTT Exports</li>
          </ul>
          <button
            id="plan-free-btn"
            onClick={handleFreePlanClick}
            className="btn-neo justify-center w-full font-pt-narrow font-bold text-[18px] py-3"
          >
            Start Free
          </button>
        </div>
        ...
        >
          <div className="absolute top-5 right-5 text-white/90"><SparkleIcon /></div>
          ...
          <div className="flex items-baseline gap-[6px] my-2 mb-3">
            <span className="font-bold text-[52px] text-white leading-none flex items-baseline">
              <span style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>₹</span>{proPriceDisplay}
            </span>
          ...
          <ul className="flex flex-col gap-[12px] mb-[20px] flex-grow font-pt-narrow text-[17px] text-white">
            <li className="flex items-center gap-[10px] font-bold text-[#FFE500]"><Checkmark /> Unlimited Transcripts Daily</li>
            <li className="flex items-center gap-[10px]"><Checkmark /> High-Speed Whisper Cloud AI</li>
            <li className="flex items-center gap-[10px]"><Checkmark /> 40+ Languages & Auto-Detect</li>
            <li className="flex items-center gap-[10px]"><Checkmark /> Priority Processing Queue</li>
          </ul>
          ...
          <button
            ...
            className="btn-neo-white justify-center w-full font-pt-narrow font-bold text-[18px] py-3 text-indigo"
          >
            {!authLoading && !user ? "🔒 Login to Subscribe" : "Go Unlimited Pro"}
          </button>
        </div>
        ...
        <div className="card-neo p-[32px] flex flex-col relative bg-white border-2 border-ink rounded-[20px] shadow-[4px_4px_0_#171717]">
          ...
          <div className="flex items-baseline gap-[6px] my-2 mb-3">
            <span className="font-bold text-[48px] text-indigo leading-none flex items-baseline">
              <span style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>₹</span>{teamPriceDisplay}
            </span>
          ...
          <ul className="flex flex-col gap-[12px] mb-[20px] flex-grow font-pt-narrow text-[17px] text-ink">
            <li className="flex items-center gap-[10px] font-bold text-indigo"><Checkmark /> Shared Team Workspaces</li>
            <li className="flex items-center gap-[10px]"><Checkmark /> Unlimited Transcripts Daily</li>
            <li className="flex items-center gap-[10px]"><Checkmark /> Multi-User Access Links</li>
            <li className="flex items-center gap-[10px]"><Checkmark /> Dedicated VIP Support</li>
          </ul>
          ...
          <div className="bg-white border-2 border-ink rounded-[24px] p-6 sm:p-8 max-w-lg w-full text-left shadow-[8px_8px_0_#171717] relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="mb-6">
              ...
              {user && (
                <p className="font-pt-narrow text-[13px] text-text-gray mt-1">
                  Subscribed to: <strong>{user.email}</strong>
                </p>
              )}
            </div>
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[["🟢", "GPay"], ["🟣", "PhonePe"], ["🔵", "Paytm"], ["🟠", "BHIM"]].map(([icon, name]) => (
                  <a
                    key={name}
                    href={upiPayUri}
                    target="_blank"
            ...
            <form onSubmit={handleVerifyUtr} className="pt-4 border-t border-gray-200">
              ...
              <p className="font-pt-narrow text-[13px] text-text-gray mb-3">
                Found in your UPI app under &quot;Transaction Details&quot; after payment.
              </p>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="btn-neo bg-[#FFE500] font-pt-narrow font-bold text-sm px-4 py-2 disabled:opacity-50 shrink-0"
                >
                  {verifyingUtr ? "Verifying..." : "Confirm & Activate"}
                </button>
              </div>
              {utrError && (
        ...
        <div className="fixed inset-0 bg-ink/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white border-2 border-ink rounded-[24px] p-8 max-w-md w-full text-center shadow-[8px_8px_0_#171717] animate-in fade-in zoom-in duration-200">
            ...
            <button
              ...
              className="btn-neo w-full justify-center py-3 text-[18px] bg-[#FFE500]"
            >
              Start Transcribing
            </button>
          </div>
